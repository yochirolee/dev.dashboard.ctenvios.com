...
export default function AgencyUsers({ agency_id }: { agency_id: number }) {
   ...
   const handleForgotPassword = (email: string) => {
      forgotPassword(email, {
         ...
         onError: (error) => {
            toast.error(
               (error as any).response?.data?.message || "Error al enviar el email para resetear la contraseña",
            );
         },
      });
   };
         ...
         <CardHeader>
            <div className="flex justify-between items-center gap-2">
               <CardTitle className=" flex flex-col">
                  ...
                  <p className="text-sm text-muted-foreground">Listado de usuarios de la agencia.</p>
               </CardTitle>
                  ...
                  <div className="ml-auto font-medium flex items-center gap-4">
                     ...
                     <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                           ...
                           <DropdownMenuItem onClick={() => handleForgotPassword(user.email)}>
