...
import { type Customer, type Provider, type Receiver, type Service, type ShippingRate } from "@/data/types";
...
export const useOrderStore = create<OrderStore>((set) => ({
   ...
   setSelectedService: (selectedService: ServiceWithRates | null) => {
      set({ selectedService });
      set({ shipping_rates: selectedService?.shipping_rates || ([] as ShippingRate[]) });
   },
   clearAll: () => {
      set({
         selectedCustomer: null,
